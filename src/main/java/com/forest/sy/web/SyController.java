...
/*        if("query".equals(datatype)) {
            ...
            if (!Strings.isNullOrEmpty(keyword)) {
                String likestr = String.format("'%%%s%%'", keyword);
                sqlwhere.append(" (jybh like ").append(likestr)
                        .append(" OR ypmc like ").append(likestr)
                        .append(" OR ypbh like ").append(likestr)
//                        .append(" OR pzwh like ").append(likestr)
//                        .append(" OR ph like ").append(likestr)
                        .append(" OR JYYJ like ").append(likestr)
                        .append(") ");
            }
            // 当前数据流程状态
            String currentstatus = (String) super.getItem(pageInfo, "currentstatus");
            if (!Strings.isNullOrEmpty(currentstatus)) {
                if (sqlwhere.length() > 0) {
                    sqlwhere.append(" and ");
                }
                sqlwhere.append(" currentstatus in (").append(currentstatus).append(") ");
//                FrameEmployee frameEmployee = frameEmployeeService.findBy("userid", _userid);
//                if (!ResourceUtil.BUSSINESS_UNIT_ID.equals(frameEmployee.getOrgId())){
//                    if (ResourceUtil.JINYN_REFER_NO.equals(currentstatus) || ResourceUtil.JINYN_CONFIRM_NO.equals(currentstatus)) {
//                        sqlwhere.append(" and jybm = '").append(frameEmployee.getOrgId()).append("' ");
//                    }
//                }
//                if (ResourceUtil.JINYN_NO.equals(currentstatus)) {
//                    sqlwhere.append(" and todoperson = '").append(_userid).append("' ");
//                }
            }
            // 是否盲样
            String sfmy = (String) super.getItem(pageInfo, "sfmy");
            if (!Strings.isNullOrEmpty(sfmy)) {
                if (sqlwhere.length() > 0) {
                    sqlwhere.append(" and ");
                }
                sqlwhere.append(" sfmy =").append(sfmy).append(" ");
            }
            // 是否假兽药
            String sfjsy = (String) super.getItem(pageInfo, "sfjsy");
            if (!Strings.isNullOrEmpty(sfjsy)) {
                if (sqlwhere.length() > 0) {
                    sqlwhere.append(" and ");
                }
                sqlwhere.append(" sfjsy =").append(sfjsy).append(" ");
            }
            // 统计对象
            String tjdx = (String) super.getItem(pageInfo, "tjdx");
            if (!Strings.isNullOrEmpty(tjdx)) {
                if (sqlwhere.length() > 0) {
                    sqlwhere.append(" and ");
                }
                sqlwhere.append(" tjdx =").append(tjdx).append(" ");
            }
            // 检验年度
            String jynd = (String) super.getItem(pageInfo, "jynd");
            if (!Strings.isNullOrEmpty(jynd)) {
            ...
            if (ResourceUtil.JINYN_NO.equals(nodeindex)) {
                ...
                sqlwhere.append(" todoperson = '").append(_userid).append("' ");
            }
            if (ResourceUtil.JINYN_REFER_NO.equals(nodeindex) || ResourceUtil.JINYN_CONFIRM_NO.equals(nodeindex)) {
                if (!ResourceUtil.BUSSINESS_UNIT_ID.equals(_user.getOrgId())){
                    ...
                    sqlwhere.append(" jybm = '").append(_user.getOrgId()).append("' ");
                }
            }
