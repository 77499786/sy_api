...
//        String subDictionary = ScsyReportUtil.SY_SUBDICTIONARY;
   ...
   *
   * @param doc
   * @param targetFileName pdf文件名称
   */
  private static void save2Pdf(Document doc, String targetFileName) {
    ...
    int pageNums = 0;
    try {
      pageNums = doc.getPageCount();
      Document docnew = new Document();
      PageSetup pset = docnew.getFirstSection().getPageSetup();
      pset.setRightMargin(0);
      pset.setLeftMargin(0);
      pset.setTopMargin(0);
...
//        String fname = targetFileName.replace(".pdf", "_" + i + ".jpeg");
//        doc.save(fname, iso);
        doc.save(output, iso);
        JPEGImageDecoder decoderFile = JPEGCodec.createJPEGDecoder(new ByteArrayInputStream(((ByteArrayOutputStream) output).toByteArray()));
        BufferedImage image = decoderFile.decodeAsBufferedImage();
        addImagePage(builder, image);
      }
      docnew.getFirstSection().getBody().getLastParagraph().remove();
//      docnew.save(targetFileName.replace(".pdf", ".docx"), SaveFormat.DOCX);
      docnew.save(targetFileName, SaveFormat.PDF);
    } catch (Exception ex) {
      ex.printStackTrace();
    }
  }
