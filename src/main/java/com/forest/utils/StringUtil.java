...
public class StringUtil {
	...
	public static String numRandom(int bit) {
		if (bit == 0)
			bit = 6; // 默认6位
		return String.valueOf(Math.round(Math.random()*1000000)).substring(0, bit);
//		List<String> strs = new ArrayList<>()
//		String str = "";
//		str = "0123456789";// 初始化种子
//		Joiner.on("").join(new Random().nextInt(),)
...
//		str = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijkmnpqrstuvwxyz";// 初始化种子
		return UUID.randomUUID().toString().replaceAll("-","").substring(4,4+bit);
//		return RandomStringUtils.random(bit, str);// 返回6位的字符串
	}
...
//	}
	 ...
	 */
	 ...
	 public static String alignLeft(String str, int size, String padStr) {
         ...
         if (pads == padLen) {
             return padStr.concat(str);  //str.concat(padStr);
         } else if (pads < padLen) {
             return padStr.substring(0, pads).concat(str); //str.concat(padStr.substring(0, pads));
         } else {
             ...
             return  new String(padding).concat(str);//str.concat(new String(padding));
         }
     }
}
